...
class ExitProfitCheck:
    """Comparison result for the two latest closed candles."""
...
def calculate_exit_profit_zone_tolerance(
    ...
    minimum: float = 0.0003,
    maximum: float = 0.0005,
) -> float:
...
async def run_standalone(symbol: str = "PAXG/USDT") -> None:
    """Run the 15m check directly without starting the main strategy."""
    import logging
    ...
    try:
        candles = await exchange.fetch_ohlcv(symbol, "15m", limit=250)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Standalone 15m Exit Profit Check")
    parser.add_argument("--symbol", default="PAXG/USDT")
    args = parser.parse_args()
    asyncio.run(run_standalone(args.symbol))

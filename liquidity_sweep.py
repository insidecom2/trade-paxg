...
def detect_reversal_candle(direction: str, previous: Candle, current: Candle) -> bool:
    ...
    if direction == "BUY":
        return previous_bearish and current_bullish
    return previous_bullish and current_bearish
...
def _attempt_entry(
    ...
    previous_candle = closed_5m[-2] if len(closed_5m) >= 2 else None
    if previous_candle is None or not detect_reversal_candle(direction, previous_candle, latest):
        expected_color = "แดง→เขียว" if direction == "BUY" else "เขียว→แดง"
        return _reset_state(latest), build_progress_message(
    ...
    return new_state, build_progress_message(
        symbol,
        PHASE_ENTERED,
        direction=direction,
        entry_price=entry_price,
...
def advance_liquidity_sweep_state(
    ...
    if phase == PHASE_IDLE:
        ...
        if zone is None:
            return _reset_state(latest), None
        new_state = {
            **zone.to_dict(),
        ...
        return new_state, build_progress_message(
            symbol, PHASE_NEAR_ZONE, zone_name=zone.name, zone_price=zone.price
        )
    ...
    if phase == PHASE_NEAR_ZONE:
        if detect_sweep(zone, latest):
            ...
            return new_state, build_progress_message(
                symbol, PHASE_SWEPT, zone_name=zone.name, zone_price=zone.price
            )
        if not find_nearby_zone([zone], latest.close, tolerance * 3):
            return _reset_state(latest), None
        return {**state, "last_candle_timestamp": latest.timestamp}, None
    ...
    if phase == PHASE_SWEPT:
        ...
        if latest.timestamp >= state.get("expires_at", latest.timestamp):
            return _reset_state(latest), build_progress_message(
                symbol, "SWEEP_INVALID", zone_name=zone.name
            )
        return {**state, "last_candle_timestamp": latest.timestamp}, None
    ...
    if phase == PHASE_CONFIRMING:
        ...
        return _reset_state(latest), build_progress_message(
            symbol, "CONFIRMATION_FAILED", zone_name=zone.name
        )
...
async def run_liquidity_sweep_check(symbol: str = "PAXG/USDT", now: Optional[datetime] = None) -> bool:
    ...
    try:
        ...
        new_state, message = advance_liquidity_sweep_state(analyzer, closed_5m, zones, state, symbol)
